...
from shop.product.use_cases import (
...
async def test_update_product_use_case_update_quantity(unit_of_work, product):
    ...
    dto = UpdateProductQuantityDto(
        id=product.id, action=ProductQuantityActions.ADD, amount=15
    )
...
async def test_update_product_use_case_raises_error_when_quantity_is_less_than_zero(
    unit_of_work, product
):
    use_case = UpdateProductQuantityUseCase(unit_of_work)
    dto = UpdateProductQuantityDto(
        id=product.id, action=ProductQuantityActions.SUBTRACT, amount=45
    )
