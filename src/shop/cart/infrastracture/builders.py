...
class ProductsInCartBuilder:
    def __init__(self, model: list[ProductInCartModel]) -> None:
        products = [
            ProductInCart(
                product=ProductBuilder(p.product).build(), quantity=Quantity(p.quantity)
            )
            for p in model
        ]
...
class CartBuilder:
    def __init__(self, model: CartModel) -> None:
        ...
        order_info_builder = (
            OrderInfoBuilder(model.order_info) if model.order_info else None
        )
        self._model = Cart(
            id=model.id,
